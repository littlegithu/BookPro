...
class HospitalLogin(Resource):
    def post(self):
        ...
        token = hospital.token or generate_token()
...
class HospitalDashboard(Resource):
    ...
    def get(self):
        ...
        staff_role = getattr(request, 'role', 'hospital_admin')
        if staff_role == 'Nurse':
            result['role_data'] = {
                'my_patients_count': len(today_appointments),
                'patients_seen_today': check_ins,
        ...
        if staff_role in ['Receptionist', 'Hospital Admin', 'hospital_admin']:
            result['role_data'] = result.get('role_data', {})
            result['role_data']['appointments'] = []
            result['role_data']['queue'] = []
            result['role_data']['tasks'] = []
